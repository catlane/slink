...
class Redis
{
    ...
    {
        if (!empty(Config::getInstance()->getOlinkCacheTtl())) {
            return $this->write()->setex(
                $this->getName(self::STRING_NAME . ':' . $originlink),
                Config::getInstance()->getOlinkCacheTtl(),
                $shortlink
            );
        } else {
            return $this->write()->set($this->getName(self::STRING_NAME . ':' .$originlink), $shortlink);
        }
    }
